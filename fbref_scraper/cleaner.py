...
from .config import DBDIR, DBNAME, DBRAWDIR, DBRAWNAME, MATCHTABLE, METADATATABLE
...
def loadRawData(
    dbDir: str = DBRAWDIR, dbName: str = DBRAWNAME, asDf: bool = True
) -> Tuple[List[Any] | pd.DataFrame, List[Any] | pd.DataFrame]:
...
def processMetaDataframe(
    metaDf: pd.DataFrame, dropCols: List[str] = []
) -> pd.DataFrame:
    if dropCols:
...
def processStatsDataframe(
    statsDf: pd.DataFrame, dropCols: List[str] = []
) -> pd.DataFrame:
    if dropCols:
...
def transformDataframes(metaDf: pd.DataFrame, statsDf: pd.DataFrame) -> pd.DataFrame:
    if (
        metaDf["match_url"].duplicated().any()
        or statsDf["match_url"].duplicated().any()
    ):
    ...
    df = metaDf.merge(
        statsDf, on="match_url", how="inner", validate="one_to_one"
...
def writeCleanDfToDB(
    df: pd.DataFrame,
    tableName: str = MATCHTABLE,
    dbDir: str = DBDIR,
    dbName: str = DBNAME,
...
def cleanFbrefData(
    rawDir: str = DBRAWDIR,
    rawName: str = DBRAWNAME,
    cleanDir: str = DBDIR,
    cleanName: str = DBNAME,
    mute: bool = False,
    overwrite: bool = False,
) -> pd.DataFrame:
    ...
    writeCleanDfToDB(
        df=cleanDf, dbDir=cleanDir, dbName=cleanName, mute=mute, overwrite=overwrite
    )
