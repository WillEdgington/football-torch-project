...
def parseSchedulePage(html: str) -> List[Dict[str, str | None]]:
    ...
    for row in table.select("tbody tr"):
        if "class" in row.attrs and (
            "thead" in row["class"] or "spacer" in row["class"]
        ):
            continue
        ...
        for name, schemadict in MATCHMETADATASCHEMA.items():
            if name == "match_url":
                ...
                matchLink = (
                    score.select_one("a")["href"]
                    if score and score.select_one("a")
                    else None
                )
        ...
        if not (
            matchDict["match_url"]
            and matchDict["date"]
            and matchDict["home_team"]
            and matchDict["away_team"]
        ):
            continue
...
def parseMatchPage(
    html: str, matchDict: Dict[str, str | None] = {}
) -> Dict[str, str | None]:
    soup = BeautifulSoup(html, "html.parser")

    # parse individual sections of page:
    # class="scorebox" (goals, W-D-L form, xG, Manager, Captain,
    # data/time, week, attendance, venue, officials)
    # id="field_wrap" (formation, line-up)
    # id="events_wrap" (match summary)
    # id="team_stats" (possesion, passing, shots, saves, cards)
    # id="team_stats_extra" (fouls, corners, crosses, touches, tackles, interceptions,
...
def parseScorebox(
    soup: BeautifulSoup, matchDict: Dict[str, str | None] = {}
) -> Dict[str, str | None]:
    ...
    def extractTeamInfo(teamDiv, prefix):
        ...
        matchDict[f"{prefix}_team"] = (
            cleanText(teamNameTag.text) if teamNameTag else None
        )
        ...
        for dp in datapoints:
            ...
            if "manager" in label:
                matchDict[f"{prefix}_manager"] = cleanText(
                    dp.get_text(separator="", strip=True).replace("Manager:", "")
                )
            if "captain" in label:
                capTag = dp.select_one("a")
                matchDict[f"{prefix}_captain"] = cleanText(
                    capTag.text.strip()
                    if capTag
                    else dp.get_text().replace("Captain:", "")
                )

    extractTeamInfo(teamDiv=homeDiv, prefix="home")
    extractTeamInfo(teamDiv=awayDiv, prefix="away")
    # extracting ".scorebox_meta" could be a future development;
    # however, most of that data we already get from the fixture table
...
def parseTeamStats(
    ...
    for i, key in enumerate(statKeys):
        for j, prefix in enumerate(["home", "away"]):
            if i == 0:  # possession
                possession = (
                    cleanText(statLines[i][j].get_text(strip=True))
                    if statLines[i][j]
                    else None
                )
