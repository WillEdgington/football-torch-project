...
from .config import DBDIR, DBNAME, DBRAWDIR, DBRAWNAME
    ...
    def rowExists(self, tableName: str, colValue: str, col: str) -> bool:
        try:
            row = self.conn.execute(
                f'SELECT 1 FROM "{tableName}" WHERE "{col}" = ? LIMIT 1;', (colValue,)
            )
...
class DatabaseWriter(DatabaseObject):
    ...
    def createTable(
        self, tableName: str, schema: Dict[str, Dict[str, Any]], overwrite: bool = False
    ):
        if overwrite:
        ...
        cols = ", ".join(
            [
                f"{col} {schema[col].get('type', 'TEXT')}"
