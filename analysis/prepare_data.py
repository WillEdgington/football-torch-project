...
def focalForAgainstDf(
    *calcDifFor: str, df: pd.DataFrame, colKey: str, name: str
) -> pd.DataFrame:
    df = df.copy()
    ...
    for col in calcDifFor:
        if (
            f"{col}_for" not in df.columns
            or f"{col}_against" not in df.columns
            or f"{col}_diff" in df.columns
        ):
            continue
...
def getFocalMeans(df: pd.DataFrame, nameKey: str, valKey: str) -> pd.DataFrame:
    ...
    for name in df[nameKey].unique():
        ...
        if len(focalDf[nameKey]) < 20:
            continue
        means.append({"manager": name, f"{valKey}_mean": focalDf[valKey].mean()})
...
def addRollingAvg(
    ...
    if method == "ema":
        df[newCol] = grouped.transform(
            lambda x: x.ewm(span=window, adjust=False).mean()
        )
    elif method == "gaussian":
        df[newCol] = grouped.transform(
            lambda x: x.rolling(window=window, win_type=method).mean(std=window / 4)
        )
    else:
...
def addRollingLeagueDeviation(
    ...
    df = df.copy()
    df = addRollingAvg(
        df=df, nameCol="league", valueCol=valueCol, window=2 * 10, method="simple"
    )  # last 10 matches (1 match week)
    df[f"{valueCol}_dev"] = df[f"{valueCol}"] / df[f"{valueCol}_roll_mean_league"] - 1
    df = addRollingAvg(
        df=df, nameCol=nameCol, valueCol=f"{valueCol}_dev", window=window, method=method
    )
    return df


def addRollingLeagueDevsAndDiff(
    df: pd.DataFrame,
    nameCol: str,
    valueCol: str,
    window: int = 20,
    ...
    df = df.copy()
    df = addRollingLeagueDeviation(
        df=df, nameCol=nameCol, valueCol=f"{valueCol}_for", window=window, method=method
    )
    df = addRollingLeagueDeviation(
        df=df,
        nameCol=nameCol,
        valueCol=f"{valueCol}_against",
        window=window,
        method=method,
    )
...
def getMostRecentRows(
    df: pd.DataFrame, nameCol: str, daysAgo: int | None = None
) -> pd.DataFrame:
    df = df.copy()
    df = (
...
def getMeans(
    series: pd.Series, window: int | None = None, method: str = "simple"
) -> pd.Series:
    if window is None:
        window = len(series)
...
def winProbGivenDominance(
    df: pd.DataFrame, stat: str, window: int | None = None, method: str = "simple"
) -> Dict[str, Any]:
    df = df.copy()
    ...
    homeProbSeries = getMeans(
        series=homeWinsWhenDom.astype(int), window=window, method=method
    )
    awayProbSeries = getMeans(
        series=awayWinsWhenDom.astype(int), window=window, method=method
    )
...
def getWinProbs(
    ...
    if filterCol and f"home_{filterCol}" in df.columns:
        ...
        homeResults = [
            {
                "stat": stat,
                "win_prob": winProbGivenDominance(
                    homeDf, stat, window=window, method=method
                )["home_win_prob"],
            }
            for stat in stats
            if stat != "goals"
        ...
        awayResults = [
            {
                "stat": stat,
                "win_prob": winProbGivenDominance(
                    awayDf, stat, window=window, method=method
                )["away_win_prob"],
            }
            for stat in stats
            if stat != "goals"
        ...
        results = [
            {
                ...
                "win_prob": (homeResults[i]["win_prob"] + awayResults[i]["win_prob"])
                / 2,
        ...
        results = [
            winProbGivenDominance(df, stat, window=window)
            for stat in stats
            if stat != "goals"
        ]
...
def getXYAndLinearRegression(
    ...
    assert (
        xKey is not None or x is not None
    ), "only one of xKey or x can be given as None"
    assert (
        yKey is not None or y is not None
    ), "only one of yKey or y can be given as None"
    ...
    for key in [xKey, yKey]:
        ...
        assert (
            key in df.columns
        ), f"key given for column ({key}) could not be found in the given dataframe."
...
def getLinearRegressionStats(
    df: pd.DataFrame,
    stats: List[str] | None = None,
    yKey: str = "goals_diff",
    trackSufX: List[str] = ["diff", "for", "against"],
    filterCol: str | None = None,
    filter: str = "",
) -> pd.DataFrame:
    for suf in trackSufX:
        assert suf in {
            ...
            f"invalid suffix to track for x in trackSufX ({suf}). "
            '"diff", "for", "against" are the only valid entries.'
        )
    ...
    assert (
        yKey in df.columns
    ), f"key given for y column ({yKey}) could not be found in the given dataframe."
    ...
    stats = [
        stat
        for stat in stats
        if f"{stat}_for" in df.columns and f"{stat}_against" in df.columns
    ]
    results = []
    ...
    for stat in stats:
        ...
        for suf in trackSufX:
            ...
            if suf == "diff":
                x = (
                    filteredDf[f"{stat}_for"] - filteredDf[f"{stat}_against"]
                ).values.reshape(-1, 1)
            else:
...
def getLogisticRegressionStats(
    df: pd.DataFrame,
    stats: List[str] | None = None,
    yKey: str = "win",
    trackSufX: List[str] = ["diff", "for", "against"],
    ...
    for stat in stats:
        ...
        for suf in trackSufX:
            if f"{stat}_{suf}" == "goals_diff":
                continue
            if suf == "diff":
                filteredDf[f"{stat}_diff"] = (
                    filteredDf[f"{stat}_for"] - filteredDf[f"{stat}_against"]
                )
            regsum = logisticRegressionSummary(
                df=filteredDf, xKey=f"{stat}_{suf}", yKey=yKey
            )
