...
class Normaliser:
    def __init__(
        ...
        fileDir: str | None = NORMALISERDIR,
    ):
        self.eps = eps
        self.params = {}
    ...
    def fit(self, series: Series, col: str | None = None):
        assert (
            self.train
        ), "normaliser must be in train mode to enable fitting abilities"
    ...
    def encodeSeries(
        ...
        return (
            (series - p["mean"]) / p["std"]
            if method == "standard"
            else (series - p["low"]) / (p["high"] - p["low"])
        )

    def state_dict(self) -> Dict[str, Any]:
        return {"eps": self.eps, "params": self.params}
    ...
    def saveJson(self, fileName: str | None = None, fileDir: str | None = None):
        ...
        assert fileDir is not None, (
            "Need a file directory. Set object.fileDir"
            " or pass directory name through method"
        )
        assert (
