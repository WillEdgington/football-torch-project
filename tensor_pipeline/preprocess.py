...
def tokenise(
    ...
    for col in df.columns:
        ...
        with Tokeniser(
            train=train, unkBuckets=unkBuckets, fileName=fileName, fileDir=fileDir
        ) as tkn:
...
def normalise(
    ...
    eps: float = 1e-8,
    columns: List[str] = [],
    typeFilter: str | None = "float64",
    method: str = "standard",
    fileDir: str = NORMALISERDIR,
    fileName: str = "numeric_normaliser.json",
    rememberMissing: bool = True,
) -> pd.DataFrame:
    assert method in {
        "standard",
        "minmax",
    }, 'invalid method input, Choose between: "standard", "minmax"'
    assert typeFilter is None or typeFilter in {
        "float64",
        "int64",
    }, 'typeFilter must be "float64", "int64", or None'
    typeFilters = {typeFilter} if typeFilter else {"float64", "int64"}
    df = df.copy()
    newCols = {}

    if len(columns) == 0:
        columns = [col for col in df.columns if str(df[col].dtype) in typeFilters]
    ...
    with Normaliser(eps=eps, train=train, fileName=fileName, fileDir=fileDir) as nrm:
        if train:
            for col in columns:
                ...
                if col.startswith("home_"):
                    ...
                    nrm.fit(
                        pd.concat([df[f"home_{base}"], df[f"away_{base}"]]), col=base
                    )
                    continue
                nrm.fit(df[col])
        ...
        for col in columns:
            if col not in df.columns:
                continue
            base = (
                col[5:] if col.startswith("home_") or col.startswith("away_") else col
            )
            if rememberMissing:
                newCols[f"{col}_missing"] = df[col].isna().astype(np.int8)

            filled = df[col].fillna(value=nrm.params[base]["mean"])
            newCols[f"{col}_normalised"] = nrm.encodeSeries(
                filled, col=base, method=method, fit=False
            )
...
def splitDataFrame(
    df: pd.DataFrame, trainSplit: float = 0.8
...
def getTemporalSplits(
    ...
    assert (
        0 < trainSplit < 1
    ), f"Training split (trainSplit={trainSplit}) must be in (0, 1)."
    assert (
        0 <= valSplit < 1
    ), f"Validation split (valSplit={valSplit}) must be in [0, 1)."
    assert (
        sortingCol in df.columns
    ), f"Sorting column (sortingCol={sortingCol}) is not a column in DataFrame (df)."
...
def addDaysSinceLastGameToLongDf(
    longDf: pd.DataFrame, dropHelperCols: bool = True
) -> pd.DataFrame:
    ...
    longDf["days_since_last_game"] = (longDf["date"] - longDf["prev_date"]).dt.days
...
def mergeLongDfBackToMatchDf(df: pd.DataFrame, longDf: pd.DataFrame) -> pd.DataFrame:
    ...
    assert "match_id" in longDf.columns, 'longDf requires "match_id" column.'
    assert (
        "days_since_last_game" in longDf.columns
    ), 'longDf requires "days_since_last_game" column.'
    df = df.copy()

    home = longDf.rename(columns={"days_since_last_game": "home_days_since_last_game"})[
        ["match_id", "team", "home_days_since_last_game"]
    ]
    away = longDf.rename(columns={"days_since_last_game": "away_days_since_last_game"})[
...
def matchDfToPerTeamDfs(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    assert (
        "home_team" in df.columns and "away_team" in df.columns
    ), 'df must have columns "home_team" and "away_team"'
    home = df.copy()
    ...
    teams = {
        str(team): g.sort_values("date").reset_index(drop=True).drop(columns=["team"])
...
def createY(
    df: pd.DataFrame, yCols: List[str] | str = "result", groupCol: str = "league"
    ...
    if "result" in yCols:
        ...
        df["result"] = df["goal_diff"].apply(
            lambda gd: 2 if gd > 0 else (0 if gd < 0 else 1)
        )

    matchIds = df["match_id"].to_list()
    return {
        matchIds[i]: (
            df[df["match_id"] == matchIds[i]][yCols]
...
def buildTeamWindowsV0(
    ...
    mask = np.array(
        [0] * (seqLen - 1) + [1], dtype=np.int32
    )  # atleast one row must be unmasked
    missing = (
        np.repeat(np.ones(missN, dtype=np.int8).reshape(1, -1), seqLen, axis=0)
        if missingCols
        else None
    )
...
def buildTeamWindows(
    ...
    preFeatCols = np.array(
        [int(feat in preMatchData) for feat in featureCols], dtype=np.float32
    )
    preMissCols = np.array(
        [
            int(miss.removesuffix("_missing") + "_normalised" not in preMatchData)
    ...
    for matchId in matchIds:
        ...
        windows[matchId] = (
            window.copy(),
            mask.copy(),
            missing.copy() if missing is not None else None,
        )
...
def constructTokenContCols(
    featureCols: List[str], tokeniserDir: str = TOKENISERDIR
) -> Tuple[Dict[str, List[int]], Dict[str, List[int]]]:
    tokenCols = {"index": [], "size": [], "unkBucketSize": []}
    contCols = {
        "index": [],
    }
    sizeCache = {}
...
def buildMetaData(
    ...
    if tokenCols is None or contCols is None:
        tokenCols, contCols = constructTokenContCols(
            featureCols=featureCols, tokeniserDir=tokeniserDir
        )
    meta = {
        "maxSeqLen": maxSeqLen,
        "featureCols": featureCols,
...
def buildSample(
    ...
    missingAway: np.ndarray | None = None,
...
def buildGroupedSamples(
    ...
    if yCols is None:
        yCols = [
            ...
            and str(df[col].dtype) in {"float64", "int64"}
    ...
    for matchId, homeTeam, awayTeam in zip(
        df["match_id"], df["home_team"], df["away_team"]
    ):
...
def buildAndStoreSamplesFromDfSplit(
    df: pd.DataFrame,
    ...
    if featureCols is None:
        featureCols = [
            col
            for col in df.columns
            if col.endswith("_token") or col.endswith("_normalised")
        ]
    missingCols = (
    ...
    groupedSamples = buildGroupedSamples(
        df=df,
        featureCols=featureCols,
        yCols=yCols,
        maxSeqLen=maxSeqLen,
    ...
    store = SampleStore(
        rootDir=tensorDir, shardSize=shardSize, metadata=meta, device="cpu"
    )
    storeGroupedSamples(store=store, groupedSamples=groupedSamples, split=split)
    return store


def createTrainTestDatasets(
    store: SampleStore,
    seqLen: int = 20,
    ...
    trainDs = MatchDataset(
        store=store, seqLen=seqLen, transform=trainTransform, split="train", group=group
    )
    testDs = MatchDataset(
        store=store, seqLen=seqLen, transform=None, split="test", group=group
    )
    datasets = {"train": trainDs, "test": testDs}
    if validation:
        datasets["validation"] = MatchDataset(
            store=store, seqLen=seqLen, transform=None, split="validation", group=group
        )
    return datasets


def tensorStoreFromMatchDF(
    ...
    trainDf, testDf, valDf = getTemporalSplits(
        df=df, trainSplit=trainSplit, valSplit=valSplit
    )
    buildAndStoreSamplesFromDfSplit(
        df=trainDf,
        featureCols=featureCols,
        split="train",
        tokeniserDir=tokeniserDir,
    ...
    store = buildAndStoreSamplesFromDfSplit(
        df=testDf,
        featureCols=featureCols,
        split="test",
        tokeniserDir=tokeniserDir,
...
def checkForStore(tensorDir: str = TENSORSDIR) -> None | SampleStore:
    store = SampleStore(rootDir=tensorDir, metadata={})
...
def createDataLoaders(
    tensorDict: Dict[str, MatchDataset],
    batchSize: int = 64,
    numWorkers: int = 1,
    seed: int = 42,
    pinMemory: bool = True,
    ...
    for name, ds in tensorDict.items():
        shuffle = name == "train"
        loaderDict[name] = createDataLoader(
            dataset=ds,
            batchSize=batchSize,
            shuffle=shuffle,
...
def prepareData(
    ...
    """set split=None to get a dict object containing test, train,
    and validation (if available) DataLoader objects"""
    assert split is None or split in {
        "train",
        "test",
        "validation",
    }, 'split must be None or one of "train", "test", "validation"'
    # splits = [split] if split is not None else ["train", "test", "validation"]

    store = checkForStore(tensorDir=tensorDir)
    if store is None:
        store = tensorStoreFromMatchDF(
            df=df,
    ...
    datasets = createTrainTestDatasets(
        store=store,
        seqLen=seqLen,
