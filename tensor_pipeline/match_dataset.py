...
class MatchDataset(Dataset):
    def __init__(
        ...
        assert seqLen <= self.maxSeqLen, (
            f"seqLen ({seqLen}) must be less than "
            f"or equal to maxSeqLen ({self.maxSeqLen})"
        )
        self.seqLen = seqLen
        self._seqSlice = slice(self.maxSeqLen - self.seqLen, self.maxSeqLen)
        if self.missingCols:
            assert store._sampleKeys == (
                "home",
                "away",
                "mask_home",
                "mask_away",
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        sample = {
            ...
            "missing_home": (
                missingHome[self._seqSlice] if self.missingCols is not None else None
            ),
            "missing_away": (
                missingAway[self._seqSlice] if self.missingCols is not None else None
            ),
            "y": y,
        }
    ...
    def _getIndicesFromStore(
        self, split: List[str] | str | None = None, group: List[str] | str | None = None
    ):
        ...
        for skey in self.split:
            ...
            for gkey in self.group:
                assert (
                    gkey in splitIndices
                ), f'"{gkey}" is not a valid group in store for split "{skey}"'
