...
from utils.format import hashDefinition, toJSONSafe

from .config import ConstructorFn, EvaluatorFn
from .trial import Trial


def adaptDict(original: Dict[str, Any], new: Dict[str, Any]) -> Dict[str, Any]:
...
class Evaluator:
    ...
    def run(self, trial: Trial) -> Dict[str, Any]:
        ...
        for key, dataloader in loaded["dataloaders"].items():
            evals[f"{key}"] = self.eval(
                model=loaded["model"], dataloader=dataloader, device=self.device
            )
        trial._definition = None
        return self.save(trial, evals)

    def save(self, trial: Trial, results: Dict[str, Any]) -> str:
    ...
    def _adaptDefinition(self, trial: Trial) -> Trial:
        trial._definition = adaptDict(
            original=deepcopy(trial.getDefinition()), new=self.evalDefinition
        )
        return trial

    def _loadTrial(self, trial: Trial) -> Dict[str, Any]:
        trial = self._adaptDefinition(trial)
        loaded = self.constructor(trial)
        self.device = trial._definition.get(
