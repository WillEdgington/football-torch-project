...
def _flatten(
    dic: Dict[str, Any], parentKey: str = "", sep: str = "."
) -> Dict[str, Any]:
    items = []
...
def _dropKeys(flat: Dict[str, Any], exclude: Sequence[str]) -> Dict[str, Any]:
    if not exclude:
        return flat
    return {
        k: v
...
class ExperimentResults:
    def __init__(self, root: Path | str):
        ...
        assert (
            self._trialsDir.exists()
        ), f"could not find any trials at: {self._trialsDir}"

        self._trialsJSON = self.root / "trials.json"
        assert (
            self._trialsJSON.exists()
        ), f"could not find trials.json file at: {self.root}"
    ...
    def _loadEval(self, trial: Trial, evalHash: str) -> Dict[str, Any] | None:
    ...
    def getTrial(self, trial_id: int) -> TrialResult:
    ...
    def toDataFrame(
        ...
        evalHash: str,
        split: bool = False,
        exclude: Sequence[str] = ("calibration", "confusion_matrix"),
    ...
    def addNormalisedCol(
        ...
        df["col_normalised"] = normaliseMetric(df=df, col=col, method=method, eps=eps)
    ...
    def addCompositeScore(
        ...
        return addCompositeScore(
            df=df, weights=weights, ascending=ascending, colName=colName
        )
