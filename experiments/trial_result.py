...
class TrialResult:
    ...
    def splits(self, evalHash: str) -> List[str]:
        return list(self.getEval(evalHash)["results"].keys())

    def getEval(self, evalHash: str) -> Dict[str, Any]:
        if evalHash not in self.evals:
            availableHashes = "\n  ".join(self.evalHashes())
            raise ValueError(
                f"Eval hash not found: {evalHash}\n"
                f"Available Eval hashes for trial:\n  {availableHashes}"
            )
        return self.evals[evalHash]

    def getSplit(self, evalHash: str, split: str) -> Dict[str, Any]:
        results = self.getEval(evalHash)["results"]
        if split not in results:
            availableSplits = "\n  ".join(self.splits(evalHash))
            raise ValueError(
                f"Split not found: {split}\nAvailable splits:\n  {availableSplits}"
            )
