...
def extractCalibrationSignals(
    preds: torch.Tensor, targets: torch.Tensor, probs: torch.Tensor
...
def binCalibration(
    confidences: torch.Tensor, correct: torch.Tensor, bins: int = 10
...
def computeECE(binAcc: List[float], binConf: List[float], binCount: List[int]) -> float:
...
def computeConfusionMatrix(
    numClasses: int, preds: torch.Tensor, targets: torch.Tensor
) -> torch.Tensor:
...
def evaluateMatchPredictorModel(
    model: torch.nn.Module,
    dataloader: torch.utils.data.DataLoader,
    bins: int = 20,
    device: torch.device = "cuda" if torch.cuda.is_available() else "cpu",
) -> Dict[str, Any]:
    preds, targets, probs, evalLoss = evaluateClassificationModel(
        model=model, dataloader=dataloader, getProbs=True, getLoss=True, device=device
    )

    accuracy = (preds == targets).float().mean().item()

    confMat = computeConfusionMatrix(
        numClasses=probs.shape[-1], preds=preds, targets=targets
    )

    confidences, correct = extractCalibrationSignals(
        preds=preds, targets=targets, probs=probs
    )

    binCenters, binAcc, binConf, binCount = binCalibration(
        confidences=confidences, correct=correct, bins=bins
    )

    ece = computeECE(binAcc=binAcc, binConf=binConf, binCount=binCount)
