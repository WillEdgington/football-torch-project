...
class AttentionBlock(nn.Module):
    def __init__(self, channels: int, numHeads: int = 1, dropout: float = 0.0):
        ...
        self.mha = nn.MultiheadAttention(
            embed_dim=channels, num_heads=numHeads, dropout=dropout, batch_first=True
        )
    ...
    def forward(
        ...
        x = self.ln(x)
        return self.mha(query=x, key=x, value=x, key_padding_mask=mask)[0]
...
class TokenEmbedding(nn.Module):
    def __init__(self, vocabSizes: Dict[int, int], embDim: int):
        ...
        self.embLayers = nn.ModuleList(
            [
                nn.Embedding(
                    num_embeddings=vocabSizes[i], embedding_dim=embDim, padding_idx=0
                )
...
class FeatureProjector(nn.Module):
    def __init__(
        ...
        self.contProjs = nn.ModuleList([nn.Linear(1, embDim) for _ in self.contIndexes])
    ...
    def forward(
        self, x: torch.Tensor, missing: torch.Tensor | None = None
    ) -> torch.Tensor:
        out = self.embedding(x)

        if missing is not None and self.acceptMissing:
            missing = missing.float()
            assert missing.shape[-1] == len(
                self.contIndexes
            ), "missing tensor must have shape (B, T, Continuous Features)"
            for mi, (i, cproj, mproj) in enumerate(
                zip(self.contIndexes, self.contProjs, self.missingProjs)
            ):
                ...
                out[:, :, i, :] = self.activation(
                    valProj * (1.0 - miss) + missProj * miss
                )  # missingness gate for continuous features
        else:
...
class DownsampleBlock(nn.Module):
    def __init__(
        ...
        convNorm: bool = True,
        convActivation: str = "SiLU",
        resConv: bool = True,
    ):
        assert attnBlocks > 0 or convBlocks > 0, (
            "downsample block must contain atleast one"
            " attention block or convolutional block."
        )
        super().__init__()
        ...
        for _ in range(attnBlocks):
            self.attns.append(
                Residual(
                    AttentionBlock(
                        channels=inChannels, numHeads=numAttnHeads, dropout=attnDropout
                    )
                )
                if resAttn
                else AttentionBlock(
...
class Encoder(nn.Module):
    def __init__(
        ...
        outChannels: int = 10,
        numDownBlocks: int = 1,
        attnBlocksPerDown: int = 1,
        numAttnHeads: int = 1,
        attnDropout: float = 0.0,
        resAttn: bool = True,
        convBlocksPerDown: int = 1,
        convKernelSize: int = 3,
        convNorm: bool = True,
        convActivation: str = "SiLU",
        resConv: bool = True,
        ...
        super().__init__()
        self.featProject = FeatureProjector(
        ...
        channels = [
            inChannels + (((outChannels - inChannels) * i) // numDownBlocks)
            for i in range(numDownBlocks + 1)
        ]
        ...
        for i in range(numDownBlocks):
            self.downsamples.append(
                DownsampleBlock(
                    ...
                    attnBlocks=attnBlocksPerDown,
                    numAttnHeads=numAttnHeads,
                    attnDropout=attnDropout,
                    resAttn=resAttn,
                    convBlocks=convBlocksPerDown,
                ...
                self.blocks.append(
                    Residual(
                        AttentionBlock(
                            ...
                            numHeads=numAttnHeads,
                            dropout=attnDropout,
                        )
                    )
                    if resAttn
                    else AttentionBlock(
                        channels=numFeatures, numHeads=numAttnHeads, dropout=attnDropout
                    )
                )
            ...
            if useFFN:
                self.blocks.append(
                    Residual(
                        FeedForwardBlock(
                            channels=numFeatures,
                            expansion=expansionFFN,
                            activation=activationFFN,
                            lnorm=lnormFFN,
                        )
                    )
                    if resFFN
                    else FeedForwardBlock(
                        channels=numFeatures,
                        expansion=expansionFFN,
                        activation=activationFFN,
                        lnorm=lnormFFN,
...
class MatchPredictorV0(nn.Module):
    def __init__(
        ...
        embDim: int = 2,
        acceptMissing: bool = True,
        encoderNumDownBlocks: int = 1,
        encoderAttnBlocksPerDown: int = 1,
        encoderNumAttnHeads: int = 1,
        encoderAttnDropout: float = 0.0,
        encoderResAttn: bool = True,
        encoderConvBlocksPerDown: int = 1,
        encoderConvKernelSize: int = 3,
        encoderConvNorm: bool = True,
        encoderConvActivation: str = "SiLU",
        encoderResConv: bool = True,
        featExtractorDepth: int = 2,
        featExtractorUseAttn: bool = True,
        featExtractorResAttn: bool = True,
        featExtractorAttnDropout: float = 0.0,
        featExtractorNumAttnHeads: int = 2,
        featExtractorUseFFN: bool = True,
        featExtractorResFFN: bool = True,
        featExtractorExpansionFFN: int = 2,
        featExtractorLnormFFN: bool = True,
        featExtractorActivationFFN: str = "SiLU",
        activationMLP: str | None = "SiLU",
    ):
        ...
        self.encoder = Encoder(
            ...
            embDim=embDim,
            numFeatures=numFeatures,
            outChannels=latentSize,
            numDownBlocks=encoderNumDownBlocks,
            attnBlocksPerDown=encoderAttnBlocksPerDown,
            numAttnHeads=encoderNumAttnHeads,
            attnDropout=encoderAttnDropout,
            resAttn=encoderResAttn,
            convBlocksPerDown=encoderConvBlocksPerDown,
            convKernelSize=encoderConvKernelSize,
            convNorm=encoderConvNorm,
            convActivation=encoderConvActivation,
            resConv=encoderResConv,
            acceptMissing=acceptMissing,
        )

        self.feature = FeatureExtractor(
            numFeatures=latentSize,
            depth=featExtractorDepth,
            useAttn=featExtractorUseAttn,
            resAttn=featExtractorResAttn,
            attnDropout=featExtractorAttnDropout,
            numAttnHeads=featExtractorNumAttnHeads,
            useFFN=featExtractorUseFFN,
            resFFN=featExtractorResFFN,
            expansionFFN=featExtractorExpansionFFN,
            lnormFFN=featExtractorLnormFFN,
            activationFFN=featExtractorActivationFFN,
        )
    ...
    def forward(
        ...
        xh, xa = self.encoder(xh, maskh, missingHome), self.encoder(
            xa, maska, missingAway
        )
