...
from tensor_pipeline.transforms import (
    Compose,
...
def constructTransform(name: str, params: Dict[str, Any]) -> Transform:
    match name:
        case "RandomTokenUNK":
            return RandomTokenUNK(prob=params["prob"], intensity=params["intensity"])
        case "TemporalDropout":
            return TemporalDropout(prob=params["prob"], minKeep=params["minKeep"])
        case "MissingValueAugment":
            return MissingValueAugment(
                prob=params["prob"], intensity=params["intensity"]
            )
        case "ContinuousFeatureDropout":
            return ContinuousFeatureDropout(
                prob=params["prob"], intensity=params["intensity"]
            )
        case _:
            raise RuntimeError(
                f"""{name} is not an elligable transform.
...
def prepareMatchPredictorTransform(params: Dict[str, Any] | None) -> Transform | None:
    if params is None:
        return None
    transforms = [
        constructTransform(name=key, params=val) for key, val in params.items()
    ]
    if len(transforms) > 1:
...
def constructMatchPredictorModel(
    params: Dict[str, Any],
    dataloader: Dict[str, torch.utils.data.DataLoader] | torch.utils.data.DataLoader,
    device: torch.device,
) -> torch.nn.Module:
    assert params["model"] == "MatchPredictorV0", (
        f'{params["model"]} is not an elligable model for constructor.\n'
        "Elligable Models:\n  - MatchPredictorV0"
    )
    ...
    vocabSize = {i: s for i, s in zip(tokenCols["index"], tokenCols["size"])}
    model = MatchPredictorV0(
        vocabSizes=vocabSize,
        ...
        activationMLP=params.get("activationMLP", "SiLU"),
    ).to(device=device)
    return model


def constructOptimizer(
    params: Dict[str, Any], model: torch.nn.Module
    ...
    match name:
        ...
        case "Adam":
            return torch.optim.Adam(
                params=model.parameters(), lr=params.get("lr", 0.001)
            )
        case "SGD":
        ...
        case _:
            raise RuntimeError(
                f"""{name} is not an elligable optimizer.
...
def prepareLossFunction(params: Dict[str, Any]) -> torch.nn.Module:
    ...
    match name:
        ...
        case _:
            raise RuntimeError(
                f"""{name} is not an elligable loss function.
Elligable Loss Functions:
    - CrossEntropyLoss"""
            )


def prepareMatchPredictorState(
    ...
    model = constructMatchPredictorModel(
        params=definition["model"], dataloader=dataloader, device=device
    )
    optimizer = (
        constructOptimizer(params=definition["optimizer"], model=model)
        if train
        else None
    )
    ...
    if epochsCompleted > 0:
        loadStates(
            stateName=f"MODEL_{epochsCompleted}_EPOCHS.pt",
            stateDir=modelDir,
            model=model,
            optimizer=optimizer,
        )
...
def constructMatchPredictorTrial(trial: Trial, train: bool = True) -> Dict[
    str,
    torch.nn.Module | torch.optim.Optimizer | Dict[str, torch.utils.data.DataLoader],
]:
    ...
    dataloaders = prepareMatchPredictorData(params=dataParams, seed=seed)
    loadedState = prepareMatchPredictorState(
