...
def _resolveSplits(
    trialResult: TrialResult, evalHash: str, splits: List[str] | str | None
) -> List[str]:
...
def _axGrid(
    n: int, colWidth: int = 6, rowHeight: int = 5
...
def _trialTitle(trialResult: TrialResult, trialID: int | None = None) -> str:
...
def plotLoss(
    ...
    (
        ax.set_title(f"Loss - {_trialTitle(trialResult, trialID)}")
        if owned
...
def plotAccuracy(
    ...
    ax.set_xlabel("Epochs")
    (
        ax.set_title(f"Accuracy - {_trialTitle(trialResult, trialID)}")
        if owned
...
def plotTrainingCurves(
    ...
    if suptitle is not None:
        fig.suptitle(
            f"Training Curves - {_trialTitle(trialResult, trialID)}"
            if suptitle == "default"
            else suptitle
        )
...
def plotConfusionMatrix(
    ...
    resolved = _resolveSplits(trialResult=trialResult, evalHash=evalHash, splits=splits)
    fig, axes = _axGrid(len(resolved), colWidth=6, rowHeight=5)

    for ax, split in zip(axes, resolved):
        matrix = np.array(
            trialResult.getSplit(evalHash, split)["confusion_matrix"], dtype=float
        )
        ...
        im = ax.imshow(matrix, cmap="Blues", vmin=0, vmax=1 if normalise else None)
    ...
    if suptitle is not None:
        fig.suptitle(
            f"Confusion Matrix - {_trialTitle(trialResult, trialID)}"
            if suptitle == "default"
            else suptitle
        )
...
def plotReliabilityDiagram(
    ...
    resolved = _resolveSplits(trialResult=trialResult, evalHash=evalHash, splits=splits)
    fig, axes = _axGrid(len(resolved), colWidth=6, rowHeight=6)
    ...
    if suptitle is not None:
        fig.suptitle(
            f"Reliability Diagram - {_trialTitle(trialResult, trialID)}"
            if suptitle == "default"
            else suptitle
        )
...
def plotTrialSummary(
    ...
    if totalPlots == 0:
        raise ValueError(
            "Atleast one of trainmetrics, confmat, or reliability must be True"
        )
    if evalHash is None and (confmat or reliability):
        raise ValueError(
            "For confusion matrix or reliability plots "
            "you must have an evaluator hash (evalHash)."
        )
    ...
    if trainmetrics:
        figs.append(
            plotTrainingCurves(
                trialResult=trialResult, trialID=trialID, suptitle=suptitle, show=False
            )
        )
...
def plotExperimentMetricScatter(
    xlabel: str,
    ylabel: str,
    ...
    if df is None and (experimentResults is None or evalHash is None):
        raise ValueError(
            "You must input a DataFrame (df) or an ExperimentResults "
            "object (experimentResults) and evaluator hash (evalHash)"
        )
