...
def loadResultsMap(resultsName: str, resultsDir: str, device: str = "cpu"):
    ...
    try:
        result = loadTorchObject(
            targetDir=resultsDir, fileName=resultsName, device=device
        )
        return result
    except FileNotFoundError:
        return None
